...
def generate_scripts(list_of_years,download_boolean):
    for year in list_of_years:
        ...
        with open("scraper_for_brazilian_court_decisions_for_year_"+str(year)+".R","w") as f:
...
if __name__ =="__main__":
    ...
    parser.add_argument('-dnb','--download_boolean', help='Define whether you want to download or only to convert to json.', default="TRUE")

    args = parser.parse_args()

    generate_scripts([2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022],args.download_boolean)
    all_files = os.listdir('./')
    all_r_scripts = [x for x in all_files if x.endswith('.R') and "scraper_for_brazilian_court_decisions_for_year_" in x and bool(re.search(r'\d',x))]
